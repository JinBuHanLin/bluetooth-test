...
//cachedbluetoothdevice代表一个远程蓝牙设备。它包含的设备属性（如地址、名称、RSSI等）和功能，可以对设备进行连接、断开等。
final class CachedBluetoothDevice implements Comparable<CachedBluetoothDevice> {
    ...
    static final List<LocalBluetoothProfile> mProfiles = new ArrayList<LocalBluetoothProfile>();
    // List of profiles that were previously in mProfiles, but have been removed
    private final List<LocalBluetoothProfile> mRemovedProfiles = new ArrayList<LocalBluetoothProfile>();//以前mprofiles名单，但已被删除
    ...
    final static int PERSIST_REJECTED_TIMES_LIMIT = 2;//用户有多少次拒绝连接，使被拒绝的坚持。
    ...
    private boolean mConnectAfterPairing;//只有在本地启动后，自动连接配对
    /**
    ...
    void onProfileStateChanged(LocalBluetoothProfile profile, int newProfileState) {
        ...
        {
            if (D) Log.d(TAG, " BT Turninig Off...Profile conn state change ignored...");
            return;
        }
...
//                    mLocalNapRoleConnected = true;//设备不支持 NAP，所以删除panprofile断开
//                }
            }
...
//            refresh();
//        } else if (mLocalNapRoleConnected && profile instanceof PanProfile && ((PanProfile) profile).isLocalRoleNap(mDevice) &&
...
//        }
    ...
    private void connectAutoConnectableProfiles() {
        ...
        // Reset the only-show-one-error-dialog tracking variable
        mIsConnectingErrorPossible = true;//重置唯一显示一个错误对话框跟踪变量
        for (LocalBluetoothProfile profile : mProfiles) {
     ...
     */
    void connectProfile(LocalBluetoothProfile profile) {//Connect this device to the specified profile.
        mConnectAttempted = SystemClock.elapsedRealtime();
        // Reset the only-show-one-error-dialog tracking variable
        mIsConnectingErrorPossible = true;
        connectInt(profile);
